...
predict_mama_expr <- read_tsv(file.path(BASE_PATH,"Results/PrediXcan_Output/Tejido_mama/predicted_expression.txt"), show_col_types = FALSE)
summary_mama_expr <- read_tsv(file.path(BASE_PATH,"Results/PrediXcan_Output/Tejido_mama/predicted_summary.txt"), show_col_types = FALSE)
fam <- read.table(file.path(BASE_PATH,"Results/Correcciones_Raw_data/nhs_subjects_hg19_final.fam"), header = FALSE)
capture_mama <- read_tsv(file.path(BASE_PATH,"Results/PrediXcan_Output/Tejido_sangre/predixcan_capture.tsv.gz"))
...
predict_mama_expr$IID <- sub("_.*", "", predict_mama_expr$IID)

# Convertir IID a carácter
fam$IID <- as.character(fam$IID)
predict_mama_expr$IID <- as.character(predict_mama_expr$IID)

predict_mama_expr <- predict_mama_expr %>% 
  dplyr::select(-FID)


# Eliminar duplicados si los hay
fam <- fam[!duplicated(fam$IID), ]
predict_mama_expr <- predict_mama_expr[!duplicated(predict_mama_expr$IID), ]

# Verificación previa
cat("Muestras en fam:", nrow(fam), "\n")
cat("Muestras en expresión:", nrow(predict_mama_expr), "\n")

# Renombrar genes 
genes_predichos <- colnames(predict_mama_expr)[-c(1)]
genes_sin_version <- gsub("\\..*", "", genes_predichos)


gene_map <- summary_mama_expr[, c("gene", "gene_name")]

gene_map_vector <- setNames(gene_map$gene_name, gene_map$gene)

new_colnames <- gene_map_vector[colnames(predict_mama_expr)[-1]] 
new_colnames[is.na(new_colnames)] <- colnames(predict_mama_expr)[-1][is.na(new_colnames)]
new_colnames <- make.unique(new_colnames)  
colnames(predict_mama_expr) <- c("IID", new_colnames)



# Verificar resultado
sin_renombrar <- colnames(predict_mama_expr)[-1][grepl("^ENSG", colnames(predict_mama_expr)[-1])]
...
# Union por IID 
datos_combinados <- inner_join(fam, predict_mama_expr, by = "IID")

# Verificación post-union
...
ggsave(filename = file.path(salida, "Distribución_de_número_de_SNPs_usados_por_gen_mama.png"), plot = g1, width = 8, height = 5)
...
g2 <- ggplot(res_casecontrol, aes(x = logFC, y = -log10(P.Value))) +
  ...
  theme_minimal() +
  labs(
    title = "Volcano plot - Caso vs Control (modelo_mama)",
    x = "log2 Fold Change",
    y = "-log10(P-valor)"
  )

ggsave(file.path(salida, "Volcano_plot_Caso_vs_Control_modelo_mama.png"), plot = g2, width = 8, height = 5)
...
g3 <- ggplot(df_long, aes(x = CaseControl, y = Expresion, fill = CaseControl)) +
  ...
  xlab("Grupo") +
  ggtitle("Expresión predicha de los 5 genes más significativos (modelo_mama)") +
  scale_fill_brewer(palette = "Set2")

ggsave(file.path(salida, "Resumen_de_los_5_genes_más_significativos_modelo_mama.png"), plot = g3, width = 7, height = 5)


head(res_casecontrol, 5) %>%
  kable("html", digits = 4, caption = "Resumen de los 5 genes más significativos modelo_mama") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) %>%
  save_kable(file = file.path(salida, "Tablas", "Top5_genes_significativos_mama.html"))
...
g <- ggplot(data.frame(log_varianza = log_varianzas), aes(x = log_varianza)) +
  ...
  labs(
    title = "Distribución logarítmica de varianza de expresión predicha por gen (modelo_mama)",
    x = "log10(Varianza)",
    y = "Número de genes"
  ) +
  theme_minimal(base_size = 14) +
...
ggsave(file.path(salida, "Distribución_logarítmica_de_varianza_de_expresión_predicha_por_gen_mama.png"), plot = g, width = 9, height = 5)
...
colnames_expr <- colnames(predict_mama_expr)[-1]  
new_colnames <- gene_map_vector_inv[colnames_expr]
new_colnames[is.na(new_colnames)] <- colnames_expr[is.na(new_colnames)]  # mantener si no hay match
new_colnames <- make.unique(new_colnames)  # evitar duplicados

# Reasignar nombres de columnas
colnames(predict_mama_expr) <- c("IID", new_colnames)


#Unir metadaotos y pcs a predict_mama_expr
fam <- read.table(file.path(BASE_PATH,"Results/Correcciones_Raw_data/nhs_subjects_hg19_final.fam"), header = FALSE)
colnames(fam) <- c("FID", "IID", "Father", "Mother", "Sex", "Phenotype")
...
predict_mama_expr <- left_join(predict_mama_expr, pcs, by = "IID")
predict_mama_expr <- left_join(predict_mama_expr, fam, by =  "IID")
predict_mama_expr <- left_join(predict_mama_expr, metadatos_edad, by = "IID")



genes_predichos <- predict_mama_expr %>%
...
# Asegurar que la columna de significancia esté bien definida
...
g4 <- ggplot(top_res, aes(x = logFC, y = -log10(P.Value))) +
  ...
  labs(
    title = "Análisis limma con NetActivity en tejido mama",
    x = "log Fold Change",
    y = "-log10(p-valor)",
    color = "Significancia"
  ) +
...
ggsave(file.path(salida, "Análisis_limma_con_NetActivity_en_tejido_mama.png"), plot = g4, width = 9, height = 5)
...
# Obtener parámetros de escalado
media <- attr(X_train_scaled, "scaled:center")
sd <- attr(X_train_scaled, "scaled:scale")

# Escalar test usando los mismos parámetros
X_test_scaled <- scale(X_test, center = media, scale = sd)
...
g5 <- ggplot(df, aes(x = 1 - specificity, y = sensitivity, color = dataset)) +
  ...
  theme_minimal(base_size = 14) +
  labs(title = "Curva ROC del modelo en tejido de mama (entrenamiento y test)",
...
ggsave(file.path(salida, "Curva_ROC_del_modelo_en_tejido_de_mama_entrenamiento_y_test.png"), plot = g5, width = 8, height = 5)
...
# Plot con coord_cartesian para evitar warnings
g6 <- ggplot(results_filtered, aes(x = estimate, y = reorder(term, estimate))) +
  ...
  labs(
    x = "Odds Ratio (OR)",
    y = "Pathways (GO term)",
    title = "Asociación entre vías génicas seleccionadas\n en tejido de mama y el riesgo de la enfermedad (OR e IC95%)"
  ) +
...
ggsave(file.path(salida, "Odd_tario_tejido_mama.png"), plot = g6, width = 8, height = 6)
